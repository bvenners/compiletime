...
class ExampleSpec extends MutableSpecificationClass {
  "Scala" can {
""")
...
def jar(jarFileName: String, classesDir: String) = {
  ...
  val command = List("jar", "cf", "specs2-class.jar", "-C", classesDir, ".")
  val builder = new ProcessBuilder(command)
  builder.redirectErrorStream(true)
  val start = System.currentTimeMillis
  val process = builder.start()
...
if (scalaVersion != "unknown") {
  ...
  val specs2ClassSource = new File("specs2-class.scala")
  val specs2ClassDir = new File(baseDir, "specs2-class")
  specs2ClassDir.mkdirs()
  compile(specs2ClassSource.getAbsolutePath, specs2Jar.getName + File.pathSeparator + specs2ScalazJar.getName, specs2ClassDir.getAbsolutePath)
  val specs2ClassJar = new File("specs2-class.jar")
  jar(specs2ClassJar.getName, specs2ClassDir.getAbsolutePath)
  
  val specs2Classpath = specs2ClassJar.getName + File.pathSeparator + specs2Jar.getName + File.pathSeparator + specs2ScalazJar.getName
